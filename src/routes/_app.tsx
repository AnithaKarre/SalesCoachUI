...
import { LogOut, LayoutDashboard, Users, Sparkles, ShieldCheck } from "lucide-react";
    ...
    <div className="min-h-screen bg-mesh">
      <header className="sticky top-0 z-30 border-b border-border/60 bg-background/75 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-7xl items-center gap-4 px-4 sm:px-6">
          <Link to={user.role === "Manager" ? "/manager" : "/dashboard"} className="flex items-center gap-2 font-semibold">
          ...
          <nav className="ml-2 flex items-center gap-1">
            {user.role === "DSP" ? (
              <NavItem to="/dashboard" active={pathname.startsWith("/dashboard") || pathname.startsWith("/merchants")} icon={<LayoutDashboard className="h-4 w-4" />}>
                Dashboard
              </NavItem>
            ) : (
          ...
          <div className="ml-auto flex items-center gap-3">
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-lg border border-border bg-card px-3 py-1.5 text-xs font-medium transition-all duration-300 hover:-translate-y-0.5 hover:border-primary/40 hover:text-primary hover:shadow-elegant"
            >
